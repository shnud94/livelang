...
export class LineView<T> {
    ...
    lineContainer: JQuery

    constructor(container: HTMLElement, options: LineViewOptions<T>, elementCallback: () => LineElement<T>[]) {
        $(container).empty();
    ...
    text = {
        getAllText: () => {
            return getTextInRange($(this.lineContainer).find('.text').first()[0], $(this.lineContainer).find('.text').last()[0]);
        },
        getLines: () => this.lineContainer.children(),
        getLineAndOffsetForChar: (searchIndex: number) => {
            let charCount = 0
            let foundLine = -1
            let foundOffset = -1
            this.text.getLines().toArray().some((line, lineI) => {

                const lineLength = $(line).children('.text').toArray().map(text => textOfText(text).length).sum();
                if (searchIndex >= charCount && searchIndex < charCount + lineLength) {
                    foundLine = lineI
                    foundOffset = searchIndex - charCount // TODO: this isn't right
    ...
    decorations = {
        _lineDecorations: {},
        removeAll: () => this.lineContainer.find('.decoration').remove(),
        addEndOfLineDecoration: (element: HTMLElement | string, lineNumber: number) => {
            const line = this.lineContainer.children().eq(lineNumber)
            if (line) {
                const decoration = $('<span>').addClass('decoration end-of-line').appendTo(line);
